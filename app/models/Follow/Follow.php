...
class Follow{
	...
	public function isFollowing($follower_id, $followable_id){
			...
			// create a new record
			$sql = 'SELECT EXISTS(SELECT * from ' . FOLLOW_TABLE.' WHERE follower_id = ? AND followable_id = ?) as following';
	...
	public function followFollowable($follower_id, $followable_id){
		try{
			...
			// create a new record
			$sql = 'INSERT INTO ' . FOLLOW_TABLE . ' (follower_id, followable_id) VALUES (?, ?)';
	...
	public function unfollowFollowable($follower_id, $followable_id){
		try{
			...
			// delete the record
			$sql = 'DELETE FROM ' . FOLLOW_TABLE . ' WHERE follower_id = ? AND followable_id = ?';
			$stmt = $this->con->prepare($sql);
			$stmt->execute([$follower_id, $followable_id]);
	...
	public function getFollowers($followable_id){
		try{
			...
			// delete the record
			$sql = 'SELECT follower_id FROM ' . FOLLOW_TABLE . ' WHERE followable_id = ?';
			$stmt = $this->con->prepare($sql);
	...
	public function getFollowings($follower_id){
		try{
			...
			// delete the record
			$sql = 'SELECT followable_id FROM ' . FOLLOW_TABLE . ' WHERE follower_id = ?';
			$stmt = $this->con->prepare($sql);
		...
		try{
			...
			// delete the record
			$sql = 'SELECT distinct pi.*, a.name FROM ' . FOLLOW_TABLE . ' f, product_rels pr, product_info pi, account a WHERE f.follower_id = ? AND pr.followable_id = f.followable_id and pi.id =  pr.product_id and a.followable_id = pr.followable_id';
			$stmt = $this->con->prepare($sql);
			$stmt->execute([$follower_id]);
				$products = array();
	...
	public function getFollowerCount($followable_id){
		try{
			...
			// delete the record
			$sql = 'SELECT count(follower_id) FROM ' . FOLLOW_TABLE . ' WHERE followable_id = ?';
			$stmt = $this->con->prepare($sql);
			$stmt->execute([$followable_id]);
			$result = $stmt->fetch(PDO::FETCH_ASSOC);
	...
	public function getFollowingCount($follower_id){
			...
			// delete the record
			$sql = 'SELECT count(followable_id) FROM ' . FOLLOW_TABLE . ' WHERE follower_id = ?';
			$stmt = $this->con->prepare($sql);
			$stmt->execute([$follower_id]);
			$result = $stmt->fetch(PDO::FETCH_ASSOC);
